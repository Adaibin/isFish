...
from vmf.m9.model import M9
from vmf.base_form import IdVersionForm
from vmf.base_form import IdsVersionsForm


index_dict = {'name': M9.name.contains}


class M9BaseForm(FlaskForm):
    """M9 Base"""
...
class M9IndexForm(FlaskForm):
    ...
    def index_objects(self):
        ...
        if not value:
            query = current_session.query(M9)
        else:
            cp = index_dict[self.field.data](value)
            query = current_session.query(M9).filter(cp)

        rows = query.all()
        return rows


class M9CreateForm(M9BaseForm):
    """M9 Create"""
    NAME = '添加组'

    endpoints = {'success': 'bp_m9.index',
                 'failed': ''}
    ...
    def validate_name(_, field):
        ...
        :param field:
        """
        if current_session.query(M9).filter_by(name=field.data).first():
            raise ValidationError('name is existed: (%s).' % field.data)


class M9ModifyForm(M9BaseForm, IdVersionForm):
    """M9 Modify"""
    NAME = '编辑组'
    ...
    def validate_name(form, field):
        ...
        :param field:
        """
        row = current_session.query(M9).\
            filter_by(**{'name': field.data}).first()
        if row and row.id != form.id.data:
            raise ValidationError('name is existed: (%s).' % field.data)


class M9DeleteForm(IdsVersionsForm):
    """M9 Delete"""
    NAME = '删除组'
