...
from vmf.m1.model import M1
from vmf.base_form import IdVersionForm
from vmf.base_form import IdsVersionsForm


index_dict = {'name': M1.name.contains}


class M1BaseForm(FlaskForm):
    """M1 Base"""
...
class M1IndexForm(FlaskForm):
    ...
    def index_objects(self):
        ...
        if not value:
            query = current_session.query(M1)
        else:
            cp = index_dict[self.field.data](value)
            query = current_session.query(M1).filter(cp)

        rows = query.all()
        return rows


class M1CreateForm(M1BaseForm):
    """M1 Create"""
    NAME = '添加组'

    endpoints = {'success': 'bp_m1.index',
                 'failed': ''}
    ...
    def validate_name(_, field):
        ...
        :param field:
        """
        if current_session.query(M1).filter_by(name=field.data).first():
            raise ValidationError('name is existed: (%s).' % field.data)


class M1ModifyForm(M1BaseForm, IdVersionForm):
    """M1 Modify"""
    NAME = '编辑组'
    ...
    def validate_name(form, field):
        ...
        :param field:
        """
        row = current_session.query(M1).\
            filter_by(**{'name': field.data}).first()
        if row and row.id != form.id.data:
            raise ValidationError('name is existed: (%s).' % field.data)


class M1DeleteForm(IdsVersionsForm):
    """M1 Delete"""
    NAME = '删除组'
